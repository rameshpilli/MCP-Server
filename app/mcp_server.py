...
from app.utils.generate_server_info import generate_server_info
...
async def server_info(ctx: Context) -> str:
    tools = await mcp.get_tools()
    return generate_server_info(tools, config.MCP_SERVER_HOST, config.MCP_SERVER_PORT)
...
async def process_message(message: str, context: Dict[str, Any] = None) -> str:
    ...
    try:
        ...
        if not routing_result.get("endpoints"):
            ...
            # from app.client import mcp_client  # avoid circular import
            ...
            if endpoint["type"] == "tool":
                ...
                if tool_name in tools:
                    ...
                    try:
                        ...
                        # Tools registered with @mcp.tool() decorator already have the context injected
                        # So we need to call them directly without passing ctx as a parameter
                        # The decorator handles the context injection

                        if params:
                            # Call with parameters only (context is injected by decorator)
                            result = await tool_func(**params)
                        else:
                            # Call without parameters (context is injected by decorator)
                            result = await tool_func()

                        results.append(result)
                    except TypeError as te:
                        # If we get a TypeError, it might be because the function expects ctx
                        # Try calling with ctx as first parameter
                        logger.warning(f"TypeError calling tool {tool_name}, trying with context: {str(te)}")
                        try:
                            if params:
                            ...
                            error_msg = f"Error executing tool {tool_name} (with context): {str(e2)}"
                            logger.error(error_msg, exc_info=True)
                            results.append(error_msg)
                    except Exception as e:
                        error_msg = f"Error executing tool {tool_name}: {str(e)}"
    ...
    async def handle_call_tool(request: Request):
        try:
            ...
            try:
                ...
                # If we get a TypeError, try with context
                logger.warning(f"TypeError calling tool {tool_name}, trying with context: {str(te)}")
                result = await tool_func(ctx, **parameters)
...
# uvicorn app.mcp_server:main_factory --host 0.0.0.0 --port 8080
