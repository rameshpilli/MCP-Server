...
# Confidence threshold for accepting compass tool suggestion
MIN_TOOL_CONFIDENCE = 0.1
...
class MCPBridge:
    ...
    #                         logger.warning(f"Sipping Compass hit due to malformed content: {content}")
    #                         continue
    #                     tool_name = content.get("name")
    #                     namespace = content.get("namespace")
    #
    #                     full_tool_name = f"{namespace}:{tool_name}" if namespace else tool_name
    #
    #                     # Find the actual registered tool that matches
    #                     matching_tool = None
    #                     if full_tool_name in tool_names:
    ...
    #
    #                     # if matching_tool:
    #                     #     confidence_score = float(getattr(hit, 'score', 0.7))
    #                     #     logger.info(f"Compass selected tool: {matching_tool} with confidence {confidence_score}")
    #                     #     logger.info(f"Compass selected tool: {matching_tool}")
    #                     #
    #                     #     # Extract parameters for this tool
    #                     #     params = await self._get_tool_params(matching_tool, query)
    ...
    #                     if matching_tool:
    #                         confidence_score = float(getattr(hit, 'score', 0.7))
    #                         logger.info(f"Compass selected tool: {matching_tool} (score: {confidence_score:.2f})")
    #
    #                         if confidence_score < MIN_TOOL_CONFIDENCE:
    #                             logger.info("Compass score too low — falling back to LLM/no-tool response")
    #                             return {
    #                                 "intent": "llm_direct",
    #                                 "endpoints": [],
    ...
    async def route_request(self, query: str, context: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        # 🧭 HARD OVERRIDE for known info prompts
        if "what tools" in query_lower or "available tools" in query_lower:
            logger.info("Hardcoded routing: using server_info for tool discovery queries")
            return {
                "intent": "server_info",
                "endpoints": [{
                    "type": "tool",
        ...
        if any(keyword in query_lower for keyword in ["client", "top", "revenue", "financial", "money"]):
            if "product" in query_lower:
                product_tools = [t for t in tool_names if "client_value_by_product" in t.lower()]
                if product_tools:
                    selected_tool = product_tools[0]
                    logger.info(f"Selected product tool: {selected_tool}")
                    return {
            ...
            if any(word in query_lower for word in ["time", "period", "year", "month", "quarter"]):
                time_tools = [t for t in tool_names if "client_value_by_time" in t.lower()]
                if time_tools:
                    selected_tool = time_tools[0]
                    logger.info(f"Selected time tool: {selected_tool}")
                    return {
            ...
            top_client_tools = [t for t in tool_names if "get_top_clients" in t.lower()]
            if top_client_tools:
                selected_tool = top_client_tools[0]
                logger.info(f"Selected top clients tool: {selected_tool}")
                return {
